...
class DigitalMedia; // forward declaration because library is composed of Digital Media
class LibraryIterator;

class Library {
	// Stream operator is friend class so can access fields of the Library object. 
    friend std::ostream & operator<<( std::ostream & out, Library & lib );
private:
    // declare map called media to store pointers Digital Media to implement library
    std::map<std::string, DigitalMedia *> media;
